...
const WeatherManager = {
  ...
  async fetchWeather(latitude, longitude) {
    try {
      ...
      const response = await fetch(
        `${this.API_URL}?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,relative_humidity_2m,apparent_temperature,precipitation,weather_code,wind_speed_10m,wind_direction_10m,wind_gusts_10m,pressure_msl,surface_pressure&hourly=pressure_msl&daily=sunrise,sunset&timezone=auto&forecast_days=1`
      );
  ...
  // Dernière mise à jour lisible
  getLastUpdatedLabel() {
    if (!this.cache.timestamp) return 'Jamais';
    const minutes = Math.max(0, Math.floor((Date.now() - this.cache.timestamp) / 60000));
  ...
  // Récupère la météo depuis la position GPS de l'utilisateur
  async fetchFromCurrentPosition(options = {}) {
    const { silent = false } = options;

    if (!('geolocation' in navigator)) {
      if (!silent && typeof showNotification === 'function') {
        showNotification('GPS non disponible sur cet appareil');
      }
      return null;
    }
    ...
    return new Promise((resolve) => {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          if (data && typeof App !== 'undefined' && typeof Navigation !== 'undefined' && Navigation.currentScreen === 'home') {
            App.loadHomeScreen();
          }
        ...
        (error) => {
          console.warn('Géolocalisation météo refusée/indisponible:', error);
          if (!silent && typeof showNotification === 'function') {
            showNotification('Autorisez la position pour la météo locale');
          }
          resolve(null);
        },
        {
  ...
  // Actualisation manuelle depuis l'accueil
  async refreshFromGPS() {
    if (typeof showNotification === 'function') {
      showNotification('Actualisation météo…');
    }
    const data = await this.fetchFromCurrentPosition({ silent: true });
    if (!data && typeof showNotification === 'function') {
      showNotification('Impossible de récupérer la météo');
    }
    return data;
  },
